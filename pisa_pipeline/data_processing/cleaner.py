...
class CSVCleaner:
    ...
    # def drop_correlated_columns(self, df: pd.DataFrame, threshold: float, target: Optional[str] = None) -> tuple[pd.DataFrame, list[str]]:
    #     """
    #     Drop one column from each highly correlated pair.
    #     - Only numeric columns are considered.
    #     - If target is provided, the column with lower correlation to the target is dropped.
    ...
    def detect_columns(self, df: pd.DataFrame) -> tuple[str, str, str]:
        """
        Detect key columns: score, school_id, student_id.
        - For score: return the first match.
        - For school_id and student_id: return the column with the most distinct values.
        Ignores case, spaces, special characters.
        Uses partial matching for score detection.
        """
        def normalize(text: str) -> str:
            return re.sub(r"[^a-z0-9]", "", text.lower())

        def find_col(substrings, match_any=False, select_max_unique=False):
            norm_subs = [normalize(s) for s in substrings]
            candidates = []
            for col in df.columns:
                norm_col = normalize(col)
                if match_any:
                    if all(sub in norm_col for sub in norm_subs):
                        candidates.append(col)
                else:
                    patterns = [fr"(?:^|[^a-z0-9]){re.escape(sub)}(?:[^a-z0-9]|$)" for sub in norm_subs]
                    if all(re.search(pat, col, re.IGNORECASE) for pat in patterns):
                        candidates.append(col)
            if not candidates:
                return None
            if select_max_unique:
                return max(candidates, key=lambda c: df[c].nunique())
            else:
                return candidates[0]

        # Score: match any of these keywords, return first match
        score_col = find_col(["plausible", "math"], match_any=True)
        # School ID: match all keywords, return column with most distinct values
        school_col = find_col(["school", "id"], select_max_unique=True)
        # Student ID: match all keywords, return column with most distinct values
        student_col = find_col(["student", "id"], select_max_unique=True)
